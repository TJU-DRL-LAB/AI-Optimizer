...
from networks.ppo_pevf_cl_spr_ad import PPO_PEVF
import scipy.signal
...
def main(args):
    ...
    cur_policy = PPO_PEVF(s_dim=s_dim, a_dim=a_dim, pr_dim=pr_dim, sess=sess,
                          ...
                          lr_c=0.001, lr_a=lr_a, lr_pc=lr_pc, lr_cl=args.lr_cl,
                          c_epochs=args.c_epoch, a_epochs=args.a_epoch,
                          memory_size=200000, policy_size=10000,
                          cl_tau=args.cl_tau,
                          pair_num=pair_len, sample_ratio=args.sample_ratio)
    ...
    while global_step_count < MAX_TOTAL_STEPS:
        ...
        # FIXME 0722
        # if train_cnt >= args.cl_startpoint:
        if train_cnt >= args.cl_batch_size * 1.5:
            update_num = global_step_count // args.cl_update_freq - (global_step_count - ep_step_count) // args.cl_update_freq
            for _ in range(update_num):
                cl_loss = cur_policy.update_cl(batch_size=args.cl_batch_size)
                cl_loss_his.append(cl_loss)
        ...
        if update_ep_rcd == train_interval or update_step_rcd >= 2000:
            ...
            cur_pairs = cur_policy.get_pairs()
            policy_idx = cur_policy.store_policy_data(cur_pairs)
            for traj in trajs:
                s_traj = traj[0]
                r_traj = traj[2]
                dsr_re_traj = discount(r_traj, gamma=gamma)
            ...
            if train_cnt == 0 or (train_cnt + 1) % 5 == 0:
                avg_c_loss = sum(c_loss_his[-5:]) / 5
                avg_a_loss = sum(a_loss_his[-5:]) / 5
                avg_cl_loss = sum(cl_loss_his[-100:]) / 100
                # avg_c_loss_his.append(avg_c_loss)
                # avg_a_loss_his.append(avg_a_loss)
    ...
    if is_save_data:
        print('=========================')
        print('- Saving data.')
        save_folder_path = './evaluation_data/ppo_pevf_cl_spr' + '_pf' + str(args.update_freq) \
                           + '_cf' + str(args.cl_update_freq)
        ...
        file_index = save_folder_path + env_name \
                     ...
                     + '_clbs' + str(args.cl_batch_size) \
                     + '_pl' + str(pair_len) + '_sr' + str(args.sample_ratio) \
                     + '_clmm' + str(args.cl_tau) \
                     + '_pt' + str(prev_type) + '_pclr' + str(lr_pc) + '_cllr' + str(args.lr_cl)
...
def get_args_from_parser():
    ...
    parser.add_argument("--env", type=str, default='Hopper-v1', help='env')
    ...
    parser.add_argument("--max-step", type=int, default=1000, help='total_steps(k)')
    ...
    parser.add_argument("--pair-len", type=int, default=1000, help='number of pairs for ppr')

    parser.add_argument("--lr-cl", type=float, default=0.0005, help='learning_rate_of_CL')
    parser.add_argument("--cl-update-freq", type=int, default=10, help='update_frequency_of_cl')
    parser.add_argument("--cl-batch-size", type=int, default=32, help='batch_size_of_PEVF')
    parser.add_argument("--cl-tau", type=float, default=0.01, help='update_momentum_of_cl')
    parser.add_argument("--sample-ratio", type=float, default=0.8, help='sample_ratio_of_pairs')
    # parser.add_argument("--cl-startpoint", type=int, default=50, help='startpoint_of_cl_updates')
