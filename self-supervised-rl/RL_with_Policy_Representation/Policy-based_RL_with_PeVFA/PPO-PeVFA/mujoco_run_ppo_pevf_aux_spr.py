...
from networks.ppo_pevf_aux_spr_ad import PPO_PEVF
import scipy.signal

def rollout(env, policy, a_bound, max_ep_steps=1000):
    s = env.reset()
    ep_reward = 0
    ep_step_count = 0
    t = 0
    ...
    ep_traj = [np.array(ep_s_traj), np.array(ep_a_traj), np.array(ep_r_traj)]

    return ep_reward, ep_step_count, ep_traj
...
def main(args):
    ...
    cur_policy = PPO_PEVF(s_dim=s_dim, a_dim=a_dim, pr_dim=pr_dim, sess=sess,
                          gamma=gamma, k=k, prev_type=prev_type, batch_size=128,
                          lr_c=0.001, lr_a=lr_a, lr_pc=lr_pc, lr_aux=args.lr_aux,
                          c_epochs=args.c_epoch, a_epochs=args.a_epoch,
                          memory_size=200000, policy_size=10000,
                          # aux_batch_size=128,
                          aux_batch_size=args.aux_batch_size,
                          aux_policy_batch_size=args.aux_policy_batch_size,
                          pair_data_num=args.pair_data_num,
                          pair_num=args.pair_len,
    ...
    # FIXME 0520 - pre-collection
    pre_collect_cnt = 0
    while pre_collect_cnt < 20000:
        _, _, traj = rollout(env, cur_policy, a_bound=a_bound, max_ep_steps=1000)
        s_traj = traj[0]
        r_traj = traj[2]
        ...
        for iot in range(len(s_traj)):
            ...
            cur_policy.store_transition(cur_s.tolist(), cur_dsc_re, 0)
            pre_collect_cnt += 1
            if pre_collect_cnt == 20000:
                break
    print('- Pre-collection done.')
    ...
    while global_step_count < MAX_TOTAL_STEPS:
        ...
        # FIXME 0722
        # if train_cnt >= args.aux_startpoint:
        if train_cnt >= args.aux_policy_batch_size * 1.5:
            update_num = global_step_count // args.aux_update_freq - (global_step_count - ep_step_count) // args.aux_update_freq
            for _ in range(update_num):
                aux_loss = cur_policy.update_aux()
                # aux_loss = cur_policy.update_aux(batch_size=args.aux_batch_size)
                aux_loss_his.append(aux_loss)
        ...
        if update_ep_rcd == train_interval or update_step_rcd >= 2000:
            ...
            # FIXME 0508 - calculate policy representation
            cur_pairs = cur_policy.get_pairs(is4pr=True)
            cur_pairs_data = cur_policy.get_pairs()
            policy_idx = cur_policy.store_policy_data(cur_pairs, cur_pairs_data)
            for traj in trajs:
                s_traj = traj[0]
                r_traj = traj[2]
                dsr_re_traj = discount(r_traj, gamma=gamma)
            ...
            pevf_c_loss = cur_policy.update_v(observes, disc_sum_rew, cur_pairs)
            c_loss_his.append(pevf_c_loss)

            for traj in trajs:
                t_s, t_a, t_r, _ = traj
                t_pairs = np.tile(cur_pairs, [t_s.shape[0], 1, 1])
                t_v = cur_policy.predict_v_prev(t_s, t_pairs)
            ...
            if train_cnt == 0 or (train_cnt + 1) % 5 == 0:
                avg_c_loss = sum(c_loss_his[-5:]) / 5
                avg_a_loss = sum(a_loss_his[-5:]) / 5
                avg_aux_loss = sum(aux_loss_his[-100:]) / 100
                # avg_c_loss_his.append(avg_c_loss)
                # avg_a_loss_his.append(avg_a_loss)
    ...
    if is_save_data:
        print('=========================')
        print('- Saving data.')
        save_folder_path = './evaluation_data/ppo_pevf_aux_spr' + '_pf' + str(args.update_freq) \
                           + '_af' + str(args.aux_update_freq)
        ...
        file_index = save_folder_path + env_name \
                     ...
                     + '_abs' + str(args.aux_batch_size) \
                     + '_apbs' + str(args.aux_policy_batch_size) \
                     + '_auxlr' + str(args.lr_aux) \
                     + '_pdl' + str(args.pair_data_num) \
                     + '_pl' + str(args.pair_len) + '_sr' + str(args.sample_ratio) \
...
def get_args_from_parser():
    ...
    parser.add_argument("--seed", type=int, default=4, help='random_seed')
    ...
    parser.add_argument("--pair-data-num", type=int, default=2000, help='number of pairs for aux training')
    parser.add_argument("--pair-len", type=int, default=200, help='number of pairs for ppr')
    parser.add_argument("--sample-ratio", type=float, default=0.8, help='sample_ratio_of_pairs')

    parser.add_argument("--lr-aux", type=float, default=0.001, help='learning_rate_of_AUX')
    parser.add_argument("--aux-update-freq", type=int, default=10, help='update_frequency_of_aux')
    parser.add_argument("--aux-policy-batch-size", type=int, default=16, help='batch_size_of_PEVF')
    parser.add_argument("--aux-batch-size", type=int, default=256, help='batch_size_of_PEVF')
    # parser.add_argument("--aux-startpoint", type=int, default=50, help='startpoint_of_aux_updates')
