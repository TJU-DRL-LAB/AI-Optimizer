...
class PPO_PEVF(object):
    def __init__(self, s_dim, a_dim, sess,
        ...
        self.pr_dim = self.param_dim

        # FIXME 1029 - build prev V
        if self.prev_type == 0:
            self.prev_v = self._build_value_net_concat(self.s_ph, self.param_ph, scope='PREV', trainable=True)
        elif self.prev_type == 1:
            self.prev_v = self._build_value_net_dot_prod(self.s_ph, self.param_ph, scope='PREV', trainable=True)
        elif self.prev_type == 2:
            self.prev_v = self._build_value_net_hyper(self.s_ph, self.param_ph, scope='PREV', trainable=True)
        ...
        print('-- INFO: PPO PEVF e2e w/ RPR initialized.')
        print('==========================')
