...
class PPO_PEVF(object):
    def __init__(self, s_dim, a_dim, sess,
        ...
        self.dy_a_means, self.dy_a_logvars = self._build_virtual_policy(self.s_ph, self.original_pr, trainable=True)
        ...
        print('-- INFO: PPO PEVF aux w/ OPR initialized.')
        print('==========================')
    ...
    def _build_opr_encoder(self, params, scope, reuse=False):
        ...
        p_fc1 = tf.reshape(params[:, :self.s_dim * self.k + self.k], [-1, self.s_dim + 1])
        p_fc2 = tf.reshape(params[:, self.s_dim * self.k + self.k:self.s_dim * self.k + self.k + self.k * self.k + self.k], [-1, self.k + 1])
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            e_fc1 = tf.layers.dense(e_fc1, self.pr_dim // 4, name='e3_fc1', trainable=trainable,
                                    ...
                                    kernel_initializer=tf.random_normal_initializer(
                                        stddev=np.sqrt(1 / (self.pr_dim // 2))))
            e_fc1 = tf.reduce_mean(tf.reshape(e_fc1, [-1, self.k, self.pr_dim // 4]), axis=1)
            ...
            e_fc2 = tf.layers.dense(e_fc2, self.pr_dim // 4, name='e3_fc2', trainable=trainable,
                                    ...
                                    kernel_initializer=tf.random_normal_initializer(
                                        stddev=np.sqrt(1 / (self.pr_dim // 2))))
            e_fc2 = tf.reduce_mean(tf.reshape(e_fc2, [-1, self.k, self.pr_dim // 4]), axis=1)
    ...
    def store_policy_data(self, policy_params, policy_pair_data):
        index = self.pmem_ptr % self.policy_size
        self.policy_memory[index, :] = policy_params
    ...
    def update_aux(self, batch_size=None):
        ...
        aux_loss, _ = self.sess.run([self.aux_loss, self.aux_train_op],
                                    feed_dict={self.param_ph: bparams,
                                               self.s_ph: b_s,
                                               self.a_ph: b_a,
                                               }
                                    )
    ...
    def get_pairs(self):
        indices = np.random.choice(min(self.memory_size, self.mem_ptr), size=self.pair_data_num)
        bt = self.memory[indices, :]
        bs = bt[:, :self.s_dim]
