"""
    run VDFP_ICNN/NEICNN in MuJoCo tasks
...
from networks.vdfp_icnn import VDFP as VDFP_FICNN
from networks.vdfp_neicnn import VDFP as VDFP_NEICNN
...
parser.add_argument("--icnn-type", type=int, default=1, help='type_of_ICNN')
...
if icnn_type == 0:
    vdfp = VDFP_FICNN(s_dim, a_dim, sess, gamma=gamma,
    ...
    vdfp = VDFP_NEICNN(s_dim, a_dim, sess, gamma=gamma,
                       sequence_length=seq_length, min_sequence_length=min_seq_length,
                       m_batch_size=64,
                       lr_vae=0.001, lr_r=0.0005, lr_a=0.00025,
                       m_dim=100, z_dim=50,
...
if IS_SAVE_LOG:
    ...
    var5 = tf.placeholder(tf.float32, None, name='kl_loss')
    ...
    tf.summary.scalar('ep_kl_loss', var5)

    merged = tf.summary.merge_all()
    writer = tf.summary.FileWriter('./tf_logs/' + env_name
                                   + '/vdfp_icnn/ic' + str(icnn_type)
                                   + '_m100k_b6464_m100_z50_gamma' + str(gamma) + '_lr00025x42_tg1050_'
...
r_loss_his, p_loss_his, kl_loss_his, a_loss_his = [], [], [], []
...
while global_step_count < MAX_TOTAL_STEPS:
    ...
    for ep_steps in range(MAX_EP_STEPS):
        ...
        if global_step_count > PRETRAIN_STEPS:
            ...
            recon_loss, kl_loss = vdfp.train_predictor()
            p_loss_his.append(recon_loss)
            kl_loss_his.append(kl_loss)
    ...
    print('- Episode:', ep_num, ' Reward: %i' % int(ep_reward),
          ...
          'Loss:',
          sum(r_loss_his[-100:]) / 100.0,
          sum(p_loss_his[-100:]) / 100.0,
          sum(kl_loss_his[-100:]) / 100.0,
          sum(a_loss_his[-100:]) / 100.0)
    ...
    if IS_SAVE_LOG:
        summary = sess.run(merged, feed_dict={var1: ep_reward,
                                              ...
                                              var5: sum(kl_loss_his[-100:]) / 100.0,
                                              })
