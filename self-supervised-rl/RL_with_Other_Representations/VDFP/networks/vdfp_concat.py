"""
    ...
    with Concatenation Operation ibetween condition and representation in the conditional VAE
    (VDFP_Concat)
...
class VDFP(object):
    def __init__(self, s_dim, a_dim, sess,
        ...
        print('-- INFO: VDFP_Concat initialized.')
        print('==========================')
    ...
    def _build_encoder(self, s, a, m, reuse=False):
        ...
        with tf.variable_scope('Encoder', reuse=reuse):
            concat = tf.concat([s, a, m], axis=1, name='s_a_m_concat')
            ef1 = tf.layers.dense(concat, 400, activation=tf.nn.relu, name='ef1', trainable=trainable,
                                  kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / (self.s_dim + self.a_dim + self.m_dim))))
    ...
    def _build_decoder(self, s, a, z, reuse=False):
        ...
        with tf.variable_scope('Decoder', reuse=reuse):
            concat = tf.concat([s, a, z], axis=1, name='s_a_z_concat')
            df1 = tf.layers.dense(concat, 200, activation=tf.nn.relu, name='df1', trainable=trainable,
