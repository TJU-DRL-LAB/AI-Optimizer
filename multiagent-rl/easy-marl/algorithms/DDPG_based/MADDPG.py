...
class CriticDNN(nn.Module):
    ...
    def _define_parameters(self):
        ...
        for i in range(self.args.agent_count):
            ...
            parameters_dict["fc_obs_action"] = nn.Linear(
                sum(self.args.observation_dim_list) + sum(self.args.action_dim_list), self.args.hidden_dim)
    ...
    def forward(self, observation_list, action_list):
        obs_action_cat = torch.cat(
            [torch.cat([observation_list[i], action_list[i]], dim=-1) for i in range(self.args.agent_count)], dim=-1)
        q_list = []
        for i in range(self.args.agent_count):
...
class MADDPG(object):
    ...
    def train(self, batch_experience_dict):
        ...
        # the following is the formal training logic for actor (in this version, all agents policies are updated simultaneously)
