...
class AlgoProtocol(Protocol):
    ...
    def sample_action(self, x: Union[np.ndarray, List[Any]]) -> np.ndarray:  # Union[X, Y] means either X or Y
        ...
    ...
    for total_step in xrange(1, n_steps + 1):
        ...
        with logger.measure_time("step"):
            ...
            with logger.measure_time("environment_step"):
                ...
                if 'IQL' in str(algo.impl) and 'antmaze' in env.spec.id:
                    reward -= 1
                if 'CQL' in str(algo.impl) and 'antmaze' in env.spec.id:
                    reward = (reward - 0.5) * 4
...
def train_single_env_sunrise(
    ...
    for total_step in xrange(1, n_steps + 1):
        with logger.measure_time("step"):
            ...
            # sample exploration action
            with logger.measure_time("inference"):
                action = algo.sample_action_dynamic(torch.from_numpy(fed_observation).to(device), index)
            ...
            if total_step > update_start_step and len(online_buffer) > algo.batch_size:
                if total_step % update_interval == 0:
                    # sample mini-batch
                    with logger.measure_time("sample_batch"):
                        batch = buffer.sample(
