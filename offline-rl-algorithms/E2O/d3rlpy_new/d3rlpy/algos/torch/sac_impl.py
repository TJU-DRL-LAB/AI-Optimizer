...
class SACImpl(DDPGBaseImpl):
    ...
    def _build_temperature_rlkit(self, value: float) -> None:
        self._log_temp = create_parameter((1, 1), value)
    ...
    def sample_action_2pi(self, x: torch.Tensor, agent_offline) -> torch.Tensor:
        assert self._policy is not None
        actions = []
        q_means = []

        action_online = self._policy.sample(x)
        actions.append(action_online)
        q_mean_online = self._q_func(x.unsqueeze(0), action_online.unsqueeze(0), reduction="mean").item()
        q_means.append(q_mean_online)

        action_offline = agent_offline.sample_action([x])[0]
        actions.append(torch.from_numpy(action_offline).to(x.device))
        q_mean_offline = self._q_func(x.unsqueeze(0),
                                      torch.from_numpy(action_offline).to(x.device).unsqueeze(0),
                                      reduction="mean").item()
        q_means.append(q_mean_offline)

        pw = self.boltzmann(np.array(q_means), 1/3)
