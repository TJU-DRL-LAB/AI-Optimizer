...
class AWACImpl(SACImpl):
    ...
    def _build_actor(self) -> None:
        self._policy = create_squashed_normal_policy(
            ...
            min_logstd=-6.0,
            max_logstd=0.0,
            use_std_parameter=True,  # zk modigy = False
        )
    ...
    def compute_actor_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        # unnormalize action via inverse tanh function  # 为什么要整这个？
        clipped_actions = batch.actions.clamp(-0.999999, 0.999999)  # tensor(1024, 24)
        # unnormalized_act_t = torch.atanh(clipped_actions)  # torch版本不对，改写torch.atanh()
        unnormalized_act_t = 0.5 * (torch.log(1 + clipped_actions) - torch.log(-clipped_actions + 1))
