...
from ..online.iterators import AlgoProtocol, collect, train_single_env, train_single_env_sunrise
...
class AlgoBase(LearnableBase):
    ...
    def sample_action_dynamic(self, x: torch.Tensor, index: int) -> np.ndarray:
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR
        return self._impl.sample_action_dynamic(x, index)

    def sample_action_2pi(self, x: torch.Tensor, agent_offline: AlgoProtocol) -> np.ndarray:
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR
        return self._impl.sample_action_2pi(x, agent_offline)

    def sample_action_reachability(self, x: torch.Tensor, offline_obs: np.array, total_step: int, explore_alpha: float) -> np.ndarray:
        assert self._impl is not None, IMPL_NOT_INITIALIZED_ERROR
        return self._impl.sample_action_reachability(x, offline_obs, total_step, explore_alpha)
    ...
    def fit_online_sunrise(
        self,
        env: gym.Env,
        buffer: Optional[Buffer] = None,
        online_buffer: Optional[Buffer] = None,
