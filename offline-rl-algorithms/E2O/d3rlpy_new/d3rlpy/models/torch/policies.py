...
class SquashedNormalPolicy(Policy):
    ...
    def __init__(
        ...
        if use_std_parameter:
            ...
            self._logstd = nn.Linear(encoder.get_feature_size(), action_size)  # this way
    ...
    def compute_mean_std(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        h = self._encoder(x)
        mu = self._mu(h)
        clipped_logstd = self._compute_logstd(h)
        return mu, clipped_logstd

    def mu_std(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        h = self._encoder(x)
        mu = self._mu(h)
        clipped_logstd = self._compute_logstd(h)
        return mu, clipped_logstd.exp()
    ...
    def best_action_sunrise(self, x: torch.Tensor, dist: Normal) -> torch.Tensor:
        action_dist = dist(x)
        action = cast(torch.Tensor, action_dist.probs.argmax(dim=1))
        return action
