...
![](https://latex.codecogs.com/svg.latex?\pi(s)=\\underset{\\hat{a}=a_{i}&plus;\\xi_{\\phi}\\left(s^{\\prime},&space;a_{i}\\right)}{\\operatorname{argmax}}&space;Q_{\\theta}^{0}(s,&space;\\hat{a}),&space;\\quad&space;a_{i}&space;\\sim&space;G_{\\omega}(s)&space;.)
