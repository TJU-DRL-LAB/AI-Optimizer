...
from networks.ppo_pevf_aux_opr import PPO_PEVF
import scipy.signal
...
def main(args):
    ...
    while global_step_count < MAX_TOTAL_STEPS:
        ...
        if update_ep_rcd == train_interval or update_step_rcd >= 2000:
            ...
            policy_idx = cur_policy.store_policy_data(cur_params, cur_pairs_data)
            for traj in trajs:
                s_traj = traj[0]
                r_traj = traj[2]
                dsr_re_traj = discount(r_traj, gamma=gamma)
    ...
    if is_save_data:
        print('=========================')
        print('- Saving data.')
        save_folder_path = './evaluation_data/ppo_pevf_aux_opr' + '_pf' + str(args.update_freq) \
...
def get_args_from_parser():
    ...
    parser.add_argument("--env", type=str, default='Walker2d-v1', help='env')
    ...
    parser.add_argument("--pair-data-num", type=int, default=1000, help='number of pairs for aux training')
