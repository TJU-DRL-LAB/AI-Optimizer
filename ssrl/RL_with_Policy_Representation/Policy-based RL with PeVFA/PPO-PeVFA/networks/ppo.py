...
class PPO(object):
    def __init__(self, s_dim, a_dim, sess, policy_logvar=-1.0,
        ...
        self.c_loss = 0.5 * tf.reduce_mean((self.v - self.val_ph)**2)
    ...
    def _build_value_net(self, s, scope, trainable):
        with tf.variable_scope(scope):
            fc1 = tf.layers.dense(s, 128, activation=tf.nn.relu, name='fc1', trainable=trainable,
    ...
    def update_v(self, x, y):
        ...
        for e in range(self.c_epochs):
            ...
            for j in range(num_batches):
                ...
                _, l = self.sess.run([self.c_train_op, self.c_loss], feed_dict={self.s_ph: bs,
                                                                                self.val_ph: bval
                                                                                })
