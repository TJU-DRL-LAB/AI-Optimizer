...
class PPO_PEVF(object):
    def __init__(self, s_dim, a_dim, sess,
                 ...
                 pair_num=1000, sample_ratio=0.8,
                 cl_tau=0.01,
                 pr_model=None,
                 ):
        self.sess = sess
        ...
        self.surface_pr_momentum = self._build_spr_encoder(self.pair_ph, scope='Encoder_Momentum')
        ...
        # - compute logits
        detached_original_pr_mome = tf.stop_gradient(self.surface_pr_momentum)
        Wz = tf.matmul(self.W, tf.transpose(detached_original_pr_mome, [1,0]))  # (z_dim,B)
        logits = tf.matmul(self.surface_pr, Wz)  # (B,B)
        ...
        print('-- INFO: PPO PEVF e2e w/ SPR ad CL initialized.')
        print('==========================')
    ...
    def update_cl(self, batch_size=None):
        ...
        bpairs = self.policy_memory[indices, :, :]
        ...
        cl_loss, _ = self.sess.run([self.cl_loss, self.cl_train_op], feed_dict={self.pair_ph: bpairs,
    ...
    def get_pairs(self):
        indices = np.random.choice(min(self.memory_size, self.mem_ptr), size=self.pair_num)
        bt = self.memory[indices, :]
        bs = bt[:, :self.s_dim]
