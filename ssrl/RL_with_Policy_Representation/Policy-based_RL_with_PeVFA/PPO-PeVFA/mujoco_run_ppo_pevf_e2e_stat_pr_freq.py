...
from networks.ppo_pevf_e2e_stat_pr import PPO_PEVF
import scipy.signal
...
def main(args):
    ...
    cur_policy = PPO_PEVF(s_dim=s_dim, a_dim=a_dim,  sess=sess,
    ...
    while global_step_count < MAX_TOTAL_STEPS:
        ...
        if global_step_count > args.pevf_start_step:
    ...
    if is_save_data:
        print('=========================')
        print('- Saving data.')
        save_folder_path = './evaluation_data/ppo_pevf_e2e_stat_pr_' + str(lr_a)\
                           + '_freq' + str(args.update_freq) + '_pt' + str(prev_type) \
...
def get_args_from_parser():
    ...
    parser.add_argument("--prev-type", type=int, default=3, help='structure_of_PREV')
    ...
    parser.add_argument("--pevf-start-step", type=int, default=5000, help='pre-collect steps before PeVFA training')
