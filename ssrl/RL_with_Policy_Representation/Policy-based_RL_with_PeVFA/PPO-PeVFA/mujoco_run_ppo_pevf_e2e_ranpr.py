...
from networks.ppo_pevf_e2e_ranpr import PPO_PEVF
import scipy.signal
...
def main(args):
    ...
    while global_step_count < MAX_TOTAL_STEPS:
        ...
        if update_ep_rcd == train_interval or update_step_rcd >= 2000:
            ...
            cur_pr = np.random.uniform(-1, 1, size=pr_dim)
            policy_idx = cur_policy.store_policy_data(cur_pr)
            for traj in trajs:
                s_traj = traj[0]
                r_traj = traj[2]
            ...
            pevf_c_loss = cur_policy.update_v(observes, disc_sum_rew, cur_pr)
            c_loss_his.append(pevf_c_loss)

            for traj in trajs:
                t_s, t_a, t_r, _ = traj
                t_params = np.tile(cur_pr, [t_s.shape[0], 1])
    ...
    if is_save_data:
        print('=========================')
        print('- Saving data.')
        save_folder_path = './evaluation_data/ppo_pevf_e2e_ranpr' + '_freq' + str(args.update_freq)
