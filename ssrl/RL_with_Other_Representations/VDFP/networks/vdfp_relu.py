"""
    ...
    with ReLU-activated layers for the trajectory return model
    (VDFP_ReLU)
...
class VDFP(object):
    def __init__(self, s_dim, a_dim, sess,
        ...
        print('-- INFO: VDFP_ReLU initialized.')
        print('==========================')
    ...
    def _build_r(self, m, reuse=False):
        ...
        with tf.variable_scope('Return', reuse=reuse):
            ...
            if self.slope > 0 and self.slope < 1:
                r = tf.nn.leaky_relu(r, alpha=self.slope)
