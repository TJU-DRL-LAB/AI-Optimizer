...
class VDFP(object):
    def __init__(self, s_dim, a_dim, sess,
        ...
        self.wz_weights = [v for v in r_params if 'convex_' in v.name and 'bias' not in v.name]
    ...
    # FIXME 1112 - use Negative-expanded ICNN for return model
    def _build_r(self, m, reuse=False):
        trainable = True if not reuse else False
        with tf.variable_scope('Return', reuse=reuse):
            ne_m = tf.concat([m, -m], axis=1)
            z1 = tf.layers.dense(ne_m, 32, activation=tf.nn.relu, name='convex_w1', trainable=trainable,
                                 kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / (2 * self.m_dim))))

            # w2 = tf.layers.dense(z1, 16, activation=None, name='convex_w2', trainable=trainable, use_bias=False,
            #                     kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / 32)))
            # d2 = tf.layers.dense(ne_m, 16, activation=None, name='convex_d2', trainable=trainable,
            #                     kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / (2 * self.m_dim))))
            # z2 = tf.nn.relu(w2 + d2)
            #
            # w3 = tf.layers.dense(z2, 1, activation=None, name='convex_w3', trainable=trainable, use_bias=False,
            #                     kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / 16)))
            # d3 = tf.layers.dense(ne_m, 1, activation=None, name='convex_d3', trainable=trainable,
            #                     kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / (2 * self.m_dim))))

            w3 = tf.layers.dense(z1, 1, activation=None, name='convex_w3', trainable=trainable, use_bias=False,
                                kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / 16)))
            d3 = tf.layers.dense(ne_m, 1, activation=None, name='convex_d3', trainable=trainable,
                                kernel_initializer=tf.random_normal_initializer(stddev=np.sqrt(1 / (2 * self.m_dim))))
            r = w3 + d3
