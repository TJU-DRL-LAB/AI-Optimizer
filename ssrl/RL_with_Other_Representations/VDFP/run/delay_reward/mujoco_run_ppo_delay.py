"""
    run PPO in MuJoCo tasks under delay reward settings
...
# when lamb = 0.0, it equals the vanilla PPO w/o GAE
...
ppo = PPO(s_dim, a_dim, sess, gamma=0.99, lr_c=0.001, lr_a=0.0001)
...
while global_step_count < MAX_TOTAL_STEPS:
    ...
    if (ep_num + 1) % train_interval == 0:
        ...
        a_loss = ppo.update_p(observes, actions, advantages)
        c_loss = ppo.update_v(observes, disc_sum_rew)
