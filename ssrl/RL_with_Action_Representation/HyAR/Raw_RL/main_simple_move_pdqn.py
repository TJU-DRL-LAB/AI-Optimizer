...
def pad_action(act, act_param):
    if act == 0:
        action = np.hstack(([5], act_param, [1], [0], [0], [0], [0]))
    elif act == 1:
        action = np.hstack(([5], act_param, [0], [1], [0], [0], [0]))
    elif act == 2:
        action = np.hstack(([5], act_param, [0], [0], [1], [0], [0]))
    elif act == 3:
        action = np.hstack(([5], act_param, [0], [0], [0], [1], [0]))
    elif act == 4:
        action = np.hstack(([5], act_param, [0], [0], [0], [0], [1]))
    return [action]
...
def run(args):
    ...
    from agents.pdqn_MPE import PDQNAgent
    ...
    dir = "result/PDQN/simple_move_4_direction"
    data = "0703"
    redir = os.path.join(dir, data)
    if not os.path.exists(redir):
        os.mkdir(redir)
    print("redir", redir)
    # title1 = "Reward_pdqn_simple_move_4_direction_"
    title2 = "Reward_100_pdqn_simple_move_4_direction_"
    title3 = "Test_success_pdqn_simple_move_4_direction_"
    title4 = "Test_epioside_step_pdqn_simple_move_4_direction_"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scenario', default='simple_move_4_direction', help='name of the scenario script.', type=str)
